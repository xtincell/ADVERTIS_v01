...
export default function MarketPortraitPage() {
  ...
  return (
    <div className="flex flex-col gap-6 p-4 pb-24 md:p-6">
      {/* Header */}
      <PageHeader
        title="Portrait Marché"
        description="Étude de marché par marque — Brand-Market Fit"
        backHref="/impulsion"
      ...
      {brandMarkets.length === 0 && (
        <EmptyState
          icon={BarChart3}
          title="Aucune donnée marché disponible"
          description="Lancez un audit T pour vos marques afin de générer les données Brand-Market Fit."
        />
      )}
                      ...
                      {brand.recommendations.map((rec, i) => (
                        ...
                        >
                          <span className={cn(
                            ...
                            i === 0 ? "bg-primary" : i === 1 ? "bg-blue-500" : "bg-emerald-500",
                          )}>
                            {i + 1}
                          </span>
