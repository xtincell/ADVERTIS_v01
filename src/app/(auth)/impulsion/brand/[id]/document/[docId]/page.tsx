...
export default function BrandDocumentViewerPage(props: {
  ...
  // Loading state
  if (isLoading) {
    return <PageSpinner label="Chargement du document..." />;
  }
      ...
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-3">
            <Link
              href={`/impulsion/brand/${strategyId}/generate`}
        ...
        <Card>
          <CardContent className="pt-6">
            <EmptyState
              ...
              description="Ce document ne contient aucune section."
              compact
            />
          </CardContent>
        </Card>
...
function DocumentContent({ content }: { content: string }) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (trimmed === "---") {
      flushList();
      elements.push(
        <hr key={`hr-${i}`} className="my-4 border-primary/20" />,
      );
      continue;
    }
